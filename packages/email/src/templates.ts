...
function statusPill(label: string, tone: StatusTone) {
  ...
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 18px;">
    <tr>
      <td style="border-radius:999px;background:${color.bg};padding:7px 12px;color:${color.fg};font-size:11px;line-height:1;font-weight:700;letter-spacing:.08em;text-transform:uppercase;">
        <span style="color:${color.dot};font-size:14px;vertical-align:-1px;">&#9679;</span>&nbsp;&nbsp;${escapeHtml(label)}
      </td>
    </tr>
  </table>`;
}
...
export function ctaButton(label: string, href: string) {
  ...
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:26px 0 4px;">
    <tr>
      <td align="center" bgcolor="${BRAND.red}" style="border-radius:8px;">
        <a href="${target}" style="display:inline-block;border:1px solid ${BRAND.red};border-radius:8px;background:${BRAND.red};color:#ffffff;text-decoration:none;font-size:14px;line-height:18px;font-weight:700;padding:13px 22px;">
          ${escapeHtml(label)} &nbsp;&#8594;
        </a>
      </td>
    </tr>
  ...
  const border =
    tone === "success"
      ...
      : tone === "warning"
        ? BRAND.amber
        : BRAND.red;
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:22px 0;background:${BRAND.charcoalAlt};border:1px solid ${BRAND.line};border-top:3px solid ${border};border-radius:10px;">
    <tr>
...
function featureRows(
  ...
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:22px 0 0;">${rows
    .map(
      (row, index) => `<tr>
        <td width="42" valign="top" style="padding:${index ? "14px" : "0"} 12px ${index === rows.length - 1 ? "0" : "14px"} 0;${
          index ? `border-top:1px solid ${BRAND.line};` : ""
        }">
          <div style="width:34px;height:34px;border-radius:50%;background:#3B1114;color:#FCA5A5;font-size:13px;line-height:34px;font-weight:800;text-align:center;">${escapeHtml(row.number)}</div>
        </td>
        <td valign="top" style="padding:${index ? "14px" : "0"} 0 ${index === rows.length - 1 ? "0" : "14px"};${
          index ? `border-top:1px solid ${BRAND.line};` : ""
        }">
...
function feedbackBox(feedback: string, tone: StatusTone) {
  ...
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;background:${BRAND.charcoalAlt};border-left:3px solid ${accent};border-radius:8px;">
    <tr>
...
function closingBanner(title: string, body: string) {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:28px 0 0;background:${BRAND.black};border:1px solid ${BRAND.line};border-radius:10px;">
    <tr>
      <td align="center" style="padding:19px 18px;">
...
<body style="margin:0;padding:0;background:${BRAND.black};color:${BRAND.white};font-family:Arial,'Segoe UI',Helvetica,sans-serif;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;">
  ${preheader}
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${BRAND.black}" style="width:100%;background:${BRAND.black};">
    <tr>
      <td align="center" style="padding:30px 12px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:600px;">
          <tr>
            <td align="center" style="padding:24px 24px 22px;background:${BRAND.black};border:1px solid ${BRAND.line};border-bottom:3px solid ${BRAND.red};border-radius:14px 14px 0 0;">
              ...
              <p style="margin:10px 0 0;color:${BRAND.muted};font-size:10px;line-height:15px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;">Learn. Build. Launch. Grow.</p>
            </td>
          </tr>
          <tr>
            <td style="padding:34px 34px 32px;background:${BRAND.charcoal};border-right:1px solid ${BRAND.line};border-left:1px solid ${BRAND.line};">
          ...
          <tr>
            <td align="center" style="padding:23px 26px;background:${BRAND.black};border:1px solid ${BRAND.line};border-top:0;border-radius:0 0 14px 14px;">
...
export function passwordOtpEmailHtml(params: {
  ...
  return emailLayout({
    ...
    bodyHtml: `
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0;background:${BRAND.charcoalAlt};border:1px solid ${BRAND.line};border-top:3px solid ${BRAND.red};border-radius:10px;">
        <tr>
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0;background:#261D0D;border-left:3px solid ${BRAND.amber};border-radius:8px;">
        <tr>
          <td style="padding:13px 15px;color:#FDE68A;font-size:13px;line-height:20px;">
            This code expires in <strong>${params.expiresMinutes} minutes</strong> and can only be used once.
          </td>
        </tr>
      </table>
