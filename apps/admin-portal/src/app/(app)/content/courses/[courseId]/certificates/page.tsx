...
export default async function CourseCertificatesPage({
  ...
  const issuedSample = certificates[0];
  const previewName = issuedSample?.student?.name ?? "Alex Morgan";
  const previewIssued = issuedSample
    ? formatDate(issuedSample.issued_at)
    : "August 25, 2026";

  return (
    <div className="space-y-4">
      <AdminPanel title="Issued certificate design">
        <p className="mb-4 text-sm text-muted">
          This is the ScaleX certificate issued to students. Landscape A4 with
          the red border — generated by{" "}
          <code className="text-xs">generateCertificatePdf</code>. There is no
          second design system.
        </p>
        <CertificateDesignPreview
          studentName={previewName}
          courseTitle={course.title}
          issuedLabel={previewIssued}
        />
        <div className="mt-4">
          <a
            href={`/content/courses/${courseId}/certificates/preview`}
            target="_blank"
            rel="noreferrer"
